...
// Add this function definition near your other GUI output functions:
void GUI::DrawCounts(int N, int V, int G, int I, int D)
{
	int L = RestWidth / 2;

	// Set color and font for the count text
	pWind->SetPen(YELLOW);
	pWind->SetFont(16, BOLD, BY_NAME, "Arial");

	// WAITING REGION (Top-Left)
	// Prints the count for Normal (N), VIP (V), and Vegan (G) waiting orders.
	// The offset of +25 places it below the "WAIT" label.
	string wait_msg = "N:" + to_string(N) + " V:" + to_string(V) + " G:" + to_string(G);
	pWind->DrawString(RestStartX + (int)(0.1 * L), RestStartY + 5 * L / 12 + 25, wait_msg);

	// IN-SERVICE REGION (Bottom-Right, labeled SRVG)
	// Prints the count for orders currently in service.
	// Replace "SRVG" with the appropriate label for In-Service count.
	string srv_msg = "SRV:" + to_string(I);
	pWind->DrawString(WindWidth / 2 + (int)(0.1 * L), YHalfDrawingArea + 5 * L / 12 + 25, srv_msg);

	// DONE REGION (Bottom-Left)
	string done_msg = "DONE:" + to_string(D);
	pWind->DrawString(RestStartX + (int)(0.1 * L), YHalfDrawingArea + 5 * L / 12 + 25, done_msg);

	// COOK REGION (Top-Right)
	// Add cook counts here later (N_C, V_C, G_C)
	pWind->DrawString(WindWidth / 2 + (int)(0.1 * L), RestStartY + 5 * L / 12 + 25, "Cooks: TBD");
}
